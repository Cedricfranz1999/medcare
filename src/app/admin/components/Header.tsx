...
const Header = () => {
  ...
  const { data: lowStockCountData, refetch: refetchLowStockCount } =
    api.medicine.getLowStockCount.useQuery();
  // Fetch the count of near expiry medicines with notificationopen: true
  const { data: nearExpiryCountData, refetch: refetchNearExpiryCount } =
    api.medicine.getNearExpiryCount.useQuery();
  // Fetch the count of expired medicines with notificationopen: true
  const { data: expiredCountData, refetch: refetchExpiredCount } =
    api.medicine.getExpiredCount.useQuery();
  // Fetch the list of all low stock medicines
  const { data: lowStockData, refetch: refetchLowStock } =
    api.medicine.getLowStock.useQuery();
  // Fetch the list of all near expiry medicines
  const { data: nearExpiryData, refetch: refetchNearExpiry } =
    api.medicine.getNearExpiry.useQuery();
  // Fetch the list of all expired medicines
  const { data: expiredData, refetch: refetchExpired } =
    api.medicine.getExpired.useQuery();
  // Mutation to update notificationopen for low stock medicines
  const { mutate: updateLowStockNotificationOpen } =
    api.medicine.updateLowStockNotificationOpen.useMutation();
  // Mutation to update notificationopen for near expiry medicines
  const { mutate: updateNearExpiryNotificationOpen } =
    api.medicine.updateNearExpiryNotificationOpen.useMutation();
  // Mutation to update notificationopen for expired medicines
  const { mutate: updateExpiredNotificationOpen } =
    api.medicine.updateNearExpiryNotificationOpen.useMutation();
          ...
          >
            ...
            {activeTab === "lowStock" ? (
              lowStockMedicines.length > 0 ? (
                ...
                <DropdownMenuItem className="cursor-pointer justify-center text-[#156cbc]">
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        <span
                          className={`ml-2 text-xs font-medium whitespace-nowrap ${expiryStatus.color}`}
                        >
                          {expiryStatus.text}
                        </span>
                      </div>
              ...
              expiredMedicines.map((medicine) => (
                ...
                >
                  ...
                  >
                    ...
                    <span className="ml-2 text-xs font-medium whitespace-nowrap text-red-600">
                      Expired
                    </span>
                  </div>
                </DropdownMenuItem>
