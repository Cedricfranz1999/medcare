  ...
  try {
    ...
    const responseData = {
      message: isBulkRequest 
        ? `Successfully created ${results.length} medicine request(s) for ${userIds.length} user(s)`
        : `Successfully created medicine request with ${results[0]?.items.length} medicine(s)`,
      ...
      userRequestCounts,
      totalRequestsCreated: results.length,
      totalMedicinesRequested: results.reduce((total, result) => total + result.items.length, 0),
    };
